...
/**
 * LSP client for vscode-markdown-language-server.
 *
 * Supports:
 * - textDocument/completion (headings, links, snippets)
 * - textDocument/hover
 * - textDocument/definition (link targets)
 * - textDocument/references (header references)
 * - textDocument/documentHighlight
 * - textDocument/formatting
 * - textDocument/rename (header rename)
 ...
 */
class MarkdownLanguageClient(private val manager: MarkdownLanguageServerManager) :
    ...
    override suspend fun getDefinition(uri: String, position: Position): List<Location> {
        return try {
            ...
            parseLocations(manager.sendRequest("textDocument/definition", params))
        } catch (_: Exception) {
            emptyList()
        }
    }
    ...
    override suspend fun getReferences(uri: String, position: Position): List<Location> {
        return try {
            ...
            parseLocations(manager.sendRequest("textDocument/references", params))
        } catch (_: Exception) {
            emptyList()
        }
    }
    ...
    override suspend fun rename(uri: String, position: Position, newName: String): WorkspaceEdit? {
        return try {
            ...
            val changesObj = result.asJsonObject.getAsJsonObject("changes") ?: return null
            val changes = mutableMapOf<String, List<TextEdit>>()
            for ((fileUri, editsEl) in changesObj.entrySet()) {
