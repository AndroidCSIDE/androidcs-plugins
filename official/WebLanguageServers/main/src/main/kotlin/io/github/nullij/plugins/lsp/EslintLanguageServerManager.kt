...
class EslintLanguageServerManager : PluginLanguageServerSpec {
    ...
    private var client: EslintLanguageClient? = null
    ...
    override fun start(): Boolean = runBlocking {
        ...
        initializing = true
        try {
            val binFile = File("$PREFIX/vscode-eslint-language-server")
            ...
            val builder = if (nodeFile.exists() && binFile.exists()) {
                ...
                pb.command("vscode-eslint-language-server", "--stdio")
            }
            ...
            return@runBlocking if (initialized) {
                client = EslintLanguageClient(this@EslintLanguageServerManager)
                running = true
                true
            } else {
                stopInternal()
            ...
            val params =
                JsonObject().apply {
                    ...
                    add(
                        ...
                        JsonObject().apply {
                            addProperty("nodePath", null as String?)
                            addProperty("run", "onType")
                            addProperty("validate", "on")
                            addProperty("codeAction.disableRuleComment.enable", true)
                            addProperty("codeAction.showDocumentation.enable", true)
                        },
                    )
                }
                ...
                JsonObject().apply {
                    add(
                        ...
                        JsonObject().apply {
                            ...
                            addProperty("languageId", "javascript")
