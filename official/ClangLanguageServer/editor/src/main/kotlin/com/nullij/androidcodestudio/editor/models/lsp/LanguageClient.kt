...
interface LanguageClient {
    ...
    /**
     * Organize imports for the document.
     * Sends a codeAction request with kind [CodeActionKind.SOURCE_ORGANIZE_IMPORTS]
     * and returns the resulting text edits to apply.
     */
    suspend fun organizeImports(uri: String): List<TextEdit>

    /**
     * Execute a server-side command (e.g. `jetbrains.kotlin.completion.apply`).
     * The server typically responds by sending a `workspace/applyEdit` request back.
     * Returns true if the command was sent successfully.
     * Default is a no-op — only override in clients that support workspace/executeCommand.
     */
    suspend fun executeCommand(command: LspCommand): Boolean = false

    /**
     * Execute a command and capture the resulting workspace/applyEdit response.
     * Returns only the import-line edits from the response, or empty list on timeout.
     */
