 ...
 *
 * Sends LSP requests to [BashLanguageServerManager] and parses the JSON responses into the IDE's
 * LSP model types. The implementation follows the same pattern as PythonLanguageClient.
 *
 ...
 */
class BashLanguageClient(private val manager: BashLanguageServerManager) :
    ...
    override suspend fun formatDocument(uri: String, content: String): List<TextEdit> {
        return withContext(Dispatchers.IO) {
            try {
                val process =
                    PluginApi.process.builder().command("shfmt", "-").attachStorage().launch()
                ...
                if (exitCode != 0 || formatted == content) return@withContext emptyList()
                ...
                listOf(
                    TextEdit(
                        range =
                            Range(
                                start = Position(0, 0),
                                end = Position(lines.size - 1, lastLine.length),
                            ),
                        newText = formatted,
                    )
                )
            } catch (e: Exception) {
                Log.e(tag, "formatDocument (shfmt) error", e)
                emptyList()
            }
        }
    }
